...
const ConsultingList = () => {
    ...
    useEffect(() => {
        axios.get("./Json/mainPageTutor/consultList.json").then((response) => {
            setConsultingList(response.data?.consultList);
        ...
        if(-0.1<scrollY&&scrollY<0.1){
            if(distanceY>DivHeight){//스크롤링위치가 맨마지막에 되어있을때 item하나의 높이보다 더 드래그하면 새로고침
                ...
                for(let i = outputList.length; i < outputList.length+10; i++){
                    if(i>=ConsultingList.length){break;}
                    temp.push(ConsultingList[i]);
                }
                setOutputList(temp);
            }
        }
    ...
    const getItem = () =>{
        ...
        for(let i = 0; i < outputList.length-1; i++){
            result.push(<ConsultingItem key = {i} isEnd={false} data = {outputList[i]}></ConsultingItem>);
        }
        result.push(<ConsultingItem key = {outputList.length-1} isEnd={true} data = {outputList[outputList.length-1]} ref={ItemRef}></ConsultingItem>);
        return result;
    }
